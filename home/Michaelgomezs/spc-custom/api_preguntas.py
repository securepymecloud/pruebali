...
def get_user_data():
    ...
    if conn:
        ...
        return jsonify({"error": "Error de conexión a la base de datos en preguntas."})
##traer get data
...
def obtener_preguntas():
        ...
        if conn is None:
            return jsonify({"error": "Error de conexión a la base de datos en preguntas"}), 500
...
##michael
        ...
        if estado_actual == 2:
            ...
            if existing_response:
                # Si existe, actualiza la respuesta
                cursor.execute("UPDATE registroseguridad SET RespuestaS = %s,codRespuestasS=%s,codDiagnosticoseg =%s WHERE codRespuestasS = %s",
